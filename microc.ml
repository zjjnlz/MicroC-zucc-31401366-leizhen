...
let _ =
  ...
  | Bytecode -> let listing =
      Bytecode.string_of_prog (Compile.translate ((List.map snd (fst program)), snd program))
    in print_endline listing
  | Compile -> Execute.execute_prog (Compile.translate ((List.map snd (fst program)), snd program))
